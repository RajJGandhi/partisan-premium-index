...
2. on a live miss/failure, falls back to the newest ``ok`` row for the same ``endpoint_family``
   -> last-known-good (served with an explicit ``STALE`` status, never as if fresh).
...
def get_last_known_good(session: Session, provider_name: str, endpoint_family: str) -> Optional[ProviderCache]:
    """Newest successful response for this provider+endpoint, regardless of age."""
    return session.execute(
        select(ProviderCache)
        .where(
            ProviderCache.provider_name == provider_name,
            ProviderCache.endpoint_family == endpoint_family,
            ProviderCache.ok.is_(True),
        )
        .order_by(ProviderCache.fetched_at.desc())
        .limit(1)
