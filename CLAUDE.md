...
Automated political-data acquisition that feeds the Quant engine (spec sections 5-10, 21, 31, 42, 45). `BaseProvider` + `ProviderChain` give caching, retry/backoff, last-known-good (`provider_cache`), `provider_health`, and `data_provider_runs` audit rows; missing data is `STALE`/`EMPTY`, never zero. Chains: `election_history`, `generic_ballot`, `poll`, `candidate` — `ingest_political_data` (`make ingest` / `make ingest-offline`) writes the de-duplicated observations, `build_quant_input_from_db` + `scripts/run_quant_shadow.py --from-db` run providers → DB → engine. `contamination.py` blocks/quarantines prediction-market sources in web evidence; `markets.py` classifies Polymarket contracts (AMBIGUOUS → quarantine, never a fabricated forecast). No provider reads a market *price*. Detail: `docs/research/PPI_PROVIDERS_V1.md`. Only manual config is keys/base URLs in `.env` (Decision Desk HQ = the spec's "Decision Labs"). Live endpoints (verified 2026-08, all keyless except where noted): `votehub_race_polls` + `votehub_generic_ballot` are the primary poll / generic-ballot sources (`api.votehub.com`); `wikipedia_candidates` is the primary candidate source (one batched MediaWiki `action=query` per run → *Infobox election* D/R nominees, mirrored onto `races.dem/rep_candidate_name`; `ingest_political_data` rebuilds its `KnownRace` list from the DB after the candidate step so the poll matcher can orient head-to-heads); DDHQ polling (`polling.decisiondeskhq.com`) is the poll fallback; `decisiondesk_election_history` reads DDHQ Results API v4 `/api/v4/race-calls` (OAuth2) for state lean, disabled → seed-CSV fallback until `DECISIONDESK_CLIENT_ID`/`_SECRET`; `openfec_candidates` needs `FEC_API_KEY`. Provider last-known-good is scoped to the exact request (cache_key), so a per-race provider never serves another race's response. `scripts/check_providers.py [--probe]` (`make check-providers`) inventories every provider + reachability. With no keys, a run of the 16 marquee 2026 races yields ~14 real Quant forecasts + a couple of correct abstentions.
